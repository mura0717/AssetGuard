"""
New Assets Logger for Checking
"""
...
NEW_ASSETS_LOG_PATH = os.getenv('NEW_ASSETS_LOG_PATH', 'logs/new_asset_logs/new_assets.csv')

class NewAssetsLogger:
    def __init__(self):
        self.base_dir = Path(__file__).resolve().parents[2]
        
        path = Path(NEW_ASSETS_LOG_PATH)
        if path.is_absolute():
            self.filepath = path
        else:
    ...
    def log_new_asset(self, asset_data: Dict[str, Any], category: str):
        """Log an asset categorized as 'New' to the CSV."""
        try:
            ...
            print(f"Error writing to new assets log: {e}")
